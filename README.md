...
**退出**：托盘菜单退出时会关掉内核并把系统代理恢复原样，不会留一个指向死端口的
系统设置。

**内核不会变成孤儿**：每个拉起的 mihomo 都被放进一个 Windows 作业对象
（`KILL_ON_JOB_CLOSE`），Zephyr 无论怎么死（崩溃、被安装器强杀、任务管理器结束）
内核都跟着死。启动时还会清掉上一个实例遗留的同路径内核，只按完整路径匹配，
不碰其它 Clash 客户端的内核。安装器也加了钩子，安装和卸载前先结束本目录下的
Zephyr 和内核，覆盖安装不会再报"文件被占用"。
